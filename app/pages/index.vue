...
<template>
    <div class="space-y-8 max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        <!-- Theme Sections -->
        <section v-if="loading || Object.keys(themes).length" class="space-y-12">
            <!-- Skeleton theme sections while loading -->
            <div v-if="loading" v-for="n in 2" :key="`theme-skel-${n}`">
                <!-- Section header skeleton (matches theme title size) -->
                <div class="flex items-center gap-3 mb-6">
                    <div class="h-8 w-48 rounded-lg bg-gray-200 dark:bg-gray-700 animate-pulse" />
                </div>
                <!-- Cards skeleton -->
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                    <SkeletonAnimeCard v-for="m in 6" :key="m" />
                </div>
            </div>
