...
CREATE OR REPLACE FUNCTION "public"."get_mutual_friends"("p_friend_id" "uuid") RETURNS TABLE("friend_user_id" "uuid", "friend_name" "text", "friend_avatar" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
...
begin
  ...
  -- Caller must already be accepted friends with the profile user
  if not exists (
    select 1
    from public.friends f
  ...
  return query
  with my_friends as (
    select case when f.user_id = auth.uid() then f.friend_id else f.user_id end as oid
    from public.friends f
    where f.status = 'accepted'
      and (f.user_id = auth.uid() or f.friend_id = auth.uid())
  ),
  their_friends as (
    select case when f.user_id = p_friend_id then f.friend_id else f.user_id end as oid
    from public.friends f
    where f.status = 'accepted'
      and (f.user_id = p_friend_id or f.friend_id = p_friend_id)
  )
  select
    u.id as friend_user_id,
    coalesce(u.raw_user_meta_data->>'name', u.raw_user_meta_data->>'full_name') as friend_name,
    coalesce(u.raw_user_meta_data->>'avatar_url', u.raw_user_meta_data->>'picture') as friend_avatar
  from my_friends m
  join their_friends t on t.oid = m.oid
  join auth.users u on u.id = m.oid
  where m.oid <> auth.uid()
    and m.oid <> p_friend_id
  order by friend_name;
end;
$$;

ALTER FUNCTION "public"."get_mutual_friends"("p_friend_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."search_anime_meta"("search_query" "text", "result_limit" integer DEFAULT 30, "require_video" boolean DEFAULT false) RETURNS TABLE("source_id" bigint, "title" "text", "thumbnail" "text", "premiere_date" "date", "views" bigint, "score" numeric, "tags" "text"[], "production_company" "text")
    LANGUAGE "plpgsql" STABLE
    SET "search_path" TO 'public', 'extensions'
    AS $$
BEGIN
    search_query := nullif(trim(search_query), '');
    ...
    WHERE ARRAY[
      ...
      AND (NOT require_video OR m.video_id IS NOT NULL)
    ORDER BY pgroonga_score(m.tableoid, m.ctid) DESC
    LIMIT LEAST(GREATEST(result_limit, 1), 50);
END;
$$;

ALTER FUNCTION "public"."search_anime_meta"("search_query" "text", "result_limit" integer, "require_video" boolean) OWNER TO "postgres";
...
CREATE SEQUENCE IF NOT EXISTS "public"."new_anime_meta_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER SEQUENCE "public"."new_anime_meta_id_seq" OWNER TO "postgres";

ALTER SEQUENCE "public"."new_anime_meta_id_seq" OWNED BY "public"."anime_meta"."id";
...
ALTER TABLE ONLY "public"."anime_meta" ALTER COLUMN "id" SET DEFAULT "nextval"('"public"."new_anime_meta_id_seq"'::"regclass");
...
CREATE INDEX "anime_meta_premiere_date_idx" ON "public"."anime_meta" USING "btree" ("premiere_date");

CREATE INDEX "favorites_anime_ref_id_idx" ON "public"."favorites" USING "btree" ("anime_ref_id");

CREATE INDEX "idx_anime_meta_search_pgroonga" ON "public"."anime_meta" USING "pgroonga" ((ARRAY["title", "description", "production_company", "director"])) WITH ("tokenizer"='TokenNgram', "normalizer"='NormalizerAuto');
...
CREATE INDEX "ix_anime_meta_title" ON "public"."anime_meta" USING "pgroonga" ("title");

CREATE INDEX "search_history_user_id_idx" ON "public"."search_history" USING "btree" ("user_id");

CREATE INDEX "watch_history_anime_ref_id_idx" ON "public"."watch_history" USING "btree" ("anime_ref_id");
...
ALTER TABLE ONLY "public"."favorites"
    ADD CONSTRAINT "favorites_anime_ref_id_fkey" FOREIGN KEY ("anime_ref_id") REFERENCES "public"."anime_meta"("source_id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."watch_history"
    ADD CONSTRAINT "watch_history_anime_ref_id_fkey" FOREIGN KEY ("anime_ref_id") REFERENCES "public"."anime_meta"("source_id") ON DELETE CASCADE;
...
CREATE POLICY "Enable read access for users and friends" ON "public"."watch_history" FOR SELECT TO "authenticated" USING ((("user_id" = ( SELECT "auth"."uid"() AS "uid")) OR (EXISTS ( SELECT 1
   FROM "public"."friends" "f"
  WHERE (("f"."status" = 'accepted'::"text") AND ((("f"."user_id" = ( SELECT "auth"."uid"() AS "uid")) AND ("f"."friend_id" = "watch_history"."user_id")) OR (("f"."friend_id" = ( SELECT "auth"."uid"() AS "uid")) AND ("f"."user_id" = "watch_history"."user_id"))))))));

CREATE POLICY "Enable read for authenticated" ON "public"."anime_meta" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "Enable write for admins" ON "public"."anime_meta" TO "authenticated" USING ((( SELECT "user_roles"."role"
   FROM "public"."user_roles"
  WHERE ("user_roles"."id" = ( SELECT "auth"."uid"() AS "uid"))) = 'admin'::"text")) WITH CHECK ((( SELECT "user_roles"."role"
   FROM "public"."user_roles"
  WHERE ("user_roles"."id" = ( SELECT "auth"."uid"() AS "uid"))) = 'admin'::"text"));
...
CREATE POLICY "select_favorites" ON "public"."favorites" FOR SELECT TO "authenticated" USING ((("user_id" = ( SELECT "auth"."uid"() AS "uid")) OR (EXISTS ( SELECT 1
   FROM "public"."friends" "f"
  WHERE (("f"."status" = 'accepted'::"text") AND ((("f"."user_id" = ( SELECT "auth"."uid"() AS "uid")) AND ("f"."friend_id" = "favorites"."user_id")) OR (("f"."friend_id" = ( SELECT "auth"."uid"() AS "uid")) AND ("f"."user_id" = "favorites"."user_id"))))))));
...
REVOKE ALL ON FUNCTION "public"."create_user_settings_on_signup"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."create_user_settings_on_signup"() TO "service_role";

REVOKE ALL ON FUNCTION "public"."get_friends_with_status"() FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."get_mutual_friends"("p_friend_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."get_mutual_friends"("p_friend_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_mutual_friends"("p_friend_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_mutual_friends"("p_friend_id" "uuid") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."get_user_info"("user_id" "uuid") FROM PUBLIC;
...
GRANT ALL ON FUNCTION "public"."search_anime_meta"("search_query" "text", "result_limit" integer, "require_video" boolean) TO "anon";
GRANT ALL ON FUNCTION "public"."search_anime_meta"("search_query" "text", "result_limit" integer, "require_video" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_anime_meta"("search_query" "text", "result_limit" integer, "require_video" boolean) TO "service_role";

REVOKE ALL ON FUNCTION "public"."search_users"("search_query" "text") FROM PUBLIC;
...
GRANT ALL ON SEQUENCE "public"."new_anime_meta_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."new_anime_meta_id_seq" TO "service_role";
